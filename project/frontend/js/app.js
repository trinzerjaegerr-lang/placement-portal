...
async function saveProfile(updates) {
  ...
  return supabase.from("profiles").update(updates).eq("id", currentUser.id);
}
...
async function handleUpload(file) {
  ...
  renderUploadResult(analysis, file.name);
}
...
function renderUploadResult(data, filename) {
  ...
  if (supabase && currentProfile) {
    const update = { resume_text: filename, skills: data.skills || [], profile_completion: Math.min(100, (currentProfile.profile_completion || 0) + 25) };
...
function initProfileForm() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    await saveProfile({
      ...
      cgpa: parseFloat(document.getElementById("pCgpa").value) || 0,
    ...
    currentProfile.cgpa = parseFloat(document.getElementById("pCgpa").value) || 0;
...
function initAIAssistant() {
  ...
  fab.addEventListener("click", () => {
    ...
    if (chat.classList.contains("show") && !body.children.length) {
