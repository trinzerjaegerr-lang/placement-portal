...
async function loadApplications() {
  ...
  const { data: coApps } = await supabase.from("company_applications").select("*, company_profiles(org_name, avatar_url), jobs(job_name, role, package_lpa)").eq("student_id", currentUser.id).order("created_at", { ascending: false });
