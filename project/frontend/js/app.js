...
function renderAIKeyPrompt() {
  ...
  div.innerHTML = `<div style="border:1px dashed var(--border); border-radius:10px; padding:0.75rem;">
    <strong>Connect full AI</strong><br>
    Paste your free Gemini API key for real AI responses. Without it, I'll use a built-in rule-based reply.
    <input type="password" id="aiKeyInput" placeholder="AIza…" style="width:100%; margin:0.5rem 0; padding:0.4rem; border-radius:8px; border:1px solid var(--border); background:var(--glass-soft); color:var(--text);" />
    <button class="btn-primary btn-sm" id="aiKeySave">Save Key</button>
    <a href="https://aistudio.google.com/app/apikey" target="_blank" style="margin-left:0.5rem; font-size:0.78rem;">Get a free key</a>
  </div>`;
  body.appendChild(div);
  div.querySelector("#aiKeySave").addEventListener("click", () => {
    const val = document.getElementById("aiKeyInput").value.trim();
    if (val) {
      localStorage.setItem("GEMINI_API_KEY", val);
      div.remove();
      addBotMessage("Gemini API key saved. You'll now get real AI responses!");
    }
  });
...
async function getAIReply(question) {
  // First try the serverless function (Vercel). If it's not available (e.g. running
  // locally via plain `vite` without `vercel dev`), we fall back to a direct
  // client-side Gemini call using a key from localStorage, and finally to a
  // deterministic rule-based reply — so the assistant always works.
  try {
    ...
    if (res.ok) {
      ...
      if (data.reason === "no_api_key") {
        // Fall through to client-side attempt below.
      } else {
        // Server responded but AI failed — try client-side before giving up.
      }
    }
  } catch (err) {
    // Not deployed / not running on Vercel — fall through to client-side.
  }

  // Client-side Gemini fallback (uses a key the user pastes into the AI panel).
  const localKey = localStorage.getItem("GEMINI_API_KEY");
  if (localKey) {
    try {
      const context = {
        skills: currentProfile?.skills || [],
        cgpa: currentProfile?.cgpa || null,
        branch: currentProfile?.branch || null,
        top_matches: (allMatches || []).slice(0, 5).map((m) => ({
      ...
      const system = `You are the AI career assistant inside a college placement portal. Answer the student's question using ONLY the profile/match data given as JSON context. Be concise (under 120 words), concrete, encouraging but honest. Use simple HTML for emphasis (<strong>, <br>) — no markdown, no code fences.`;
      const upstream = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${localKey}`,
        {
          ...
          body: JSON.stringify({
            ...
            contents: [{ role: "user", parts: [{ text: `Student context: ${JSON.stringify(context)}\n\nQuestion: ${question}` }] }],
...
function ruleBasedReply(question) {
  ...
  return `<strong>I'm here to help!</strong><br><br>Ask me about skills to learn, your best company matches, how to improve your score, or your CGPA. (For full AI responses, set your Gemini API key below.)<br><br><button class="ai-suggestion" onclick="document.getElementById('aiInput').value='What skills should I learn?'; sendAIMessage();">What skills should I learn?</button>`;
}
