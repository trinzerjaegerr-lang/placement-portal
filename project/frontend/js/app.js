...
function applyRoleUI(role) {
  ...
  document.querySelectorAll('[data-role="student"]').forEach((el) => {
    el.style.display = isCompany ? "none" : (el.tagName === "A" ? "flex" : "flex");
  });
  document.querySelectorAll('[data-role="company"]').forEach((el) => {
    el.style.display = isCompany ? (el.tagName === "A" ? "flex" : "flex") : "none";
  });
...
async function initDashboard() {
  ...
  // Fallback: if profiles.role is missing, check company_profiles table
  if (currentProfile && currentProfile.role !== "company" && supabase) {
    const { data: cp } = await supabase.from("company_profiles").select("id").eq("id", currentProfile.id).maybeSingle();
    if (cp) {
      currentProfile.role = "company";
      await supabase.from("profiles").update({ role: "company" }).eq("id", currentProfile.id);
    }
  }
