...
async function becomeCompany() {
  ...
  const { error: profErr } = await supabase.from("profiles").update({ role: "company" }).eq("id", currentUser.id);
...
async function saveCompanyProfileForm(e) {
  ...
  if (payload.org_name && payload.org_name !== currentProfile.full_name) {
