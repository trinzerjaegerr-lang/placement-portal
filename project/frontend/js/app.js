...
async function loadApplications() {
  ...
  const { data: coApps } = await supabase.from("company_applications").select("*, company_profiles(org_name, avatar_url)").eq("student_id", currentUser.id).order("created_at", { ascending: false });

  const legacyRows = (data || []).map((a) => `
    <tr>
  ...
  const coRows = (coApps || []).map((a) => `
    <tr>
      <td><div class="company-cell"><div class="company-logo" style="background:#4f46e5">${(a.company_profiles?.org_name || "?").slice(0, 2).toUpperCase()}</div><div class="company-name">${a.company_profiles?.org_name || "Company"}</div></div></td>
      <td>—</td>
      <td>—</td>
      <td><span class="app-status ${a.status === "submitted" ? "applied" : a.status}">${a.status}</span></td>
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data.reason === "no_api_key") {
        return "<strong>AI isn't connected yet.</strong><br><br>Set your Gemini API key in Vercel → Settings → Environment Variables → add <code>GEMINI_API_KEY</code>, then redeploy. Get a free key at <a href=\"https://aistudio.google.com/app/apikey\" target=\"_blank\">Google AI Studio</a>.";
      }
      return "<strong>AI couldn't respond right now.</strong><br><br>Something went wrong on the server. Check that <code>GEMINI_API_KEY</code> is set correctly in Vercel Environment Variables.";
    }
  } catch (err) {
    console.warn("AI function unavailable:", err);
  }
  return "<strong>AI isn't responding.</strong><br><br>Make sure the app is deployed to Vercel and <code>GEMINI_API_KEY</code> is set in Environment Variables. Get a free key at <a href=\"https://aistudio.google.com/app/apikey\" target=\"_blank\">Google AI Studio</a>.";
}
...
function initSearch() {
  ...
  input.addEventListener("input", () => {
    ...
    document.querySelectorAll("#companyBody tr, #applicantBody tr").forEach((tr) => {
...
function populateProfileUI() {
  ...
  const roleLabel = currentProfile.role === "admin" ? "Admin" : currentProfile.role === "company" ? "Company" : "Student";
...
function initCompanyFeatures() {
  ...
  const skillsInput = document.getElementById("coSkills");
  if (skillsInput) skillsInput.addEventListener("input", renderCoSkillTags);

  const needed = document.getElementById("coEmployeesNeeded");
  const have = document.getElementById("coEmployeesHave");
  if (needed) needed.addEventListener("input", updateTrackerPreview);
  if (have) have.addEventListener("input", updateTrackerPreview);
  ...
  const pubApplyBtn = document.getElementById("pubApplyBtn");
  if (pubApplyBtn) pubApplyBtn.addEventListener("click", () => { if (currentPublicCompany) openApplyModal(currentPublicCompany); });
...
async function loadCompanyProfileForm() {
  ...
  f("coSkills").value = (currentCompanyProfile?.skills_required || []).join(", ");
  f("coEmployeesNeeded").value = currentCompanyProfile?.employees_needed || "";
  f("coEmployeesHave").value = currentCompanyProfile?.employees_have || "";
...
function renderCoSkillTags() {
  const el = document.getElementById("coSkillTags");
  if (!el) return;
  const skills = document.getElementById("coSkills").value.split(",").map((s) => s.trim()).filter(Boolean);
  el.innerHTML = skills.map((s) => `<span class="skill-tag">${s}</span>`).join("");
}

function updateTrackerPreview() {
  const needed = parseInt(document.getElementById("coEmployeesNeeded").value) || 0;
  const have = parseInt(document.getElementById("coEmployeesHave").value) || 0;
  const pct = needed > 0 ? Math.min(100, Math.round((have / needed) * 100)) : 0;
  document.getElementById("coTrackerFill").style.setProperty("--w", pct + "%");
  document.getElementById("coTrackerLabel").textContent = `${have} / ${needed} positions filled`;
}
...
async function saveCompanyProfileForm(e) {
  ...
  const payload = {
    ...
    employees_needed: parseInt(document.getElementById("coEmployeesNeeded").value) || 0,
    employees_have: parseInt(document.getElementById("coEmployeesHave").value) || 0,
  ...
  // Keep the header name in sync with the org name for company accounts.
...
async function loadCompanyOverview() {
  ...
  const { data: cp } = await supabase.from("company_profiles").select("employees_needed, employees_have").eq("id", currentUser.id).maybeSingle();
  const needed = cp?.employees_needed || 0;
  const have = cp?.employees_have || 0;
  document.getElementById("coHave").textContent = have;
  document.getElementById("coNeed").textContent = needed;
  document.getElementById("coHireBar").style.setProperty("--w", (needed > 0 ? Math.min(100, (have / needed) * 100) : 0) + "%");
...
async function loadCompanyBrowse() {
  ...
  if (!grid) return;
  if (!allCompanyProfiles.length) {
    grid.innerHTML = '<p class="muted">No company profiles yet. Check back soon.</p>';
    return;
  }
  grid.innerHTML = allCompanyProfiles.map((c) => {
    const pct = c.employees_needed > 0 ? Math.min(100, Math.round((c.employees_have / c.employees_needed) * 100)) : 0;
    return `
    <div class="company-browse-card glass-soft" data-id="${c.id}">
      ...
      <div class="cbc-body">
        ...
        <div class="tracker-bar-wrap small"><div class="tracker-bar"><span style="--w:${pct}%"></span></div><span class="muted">${c.employees_have || 0}/${c.employees_needed || 0} hired</span></div>
        <button class="btn-ghost btn-sm cbc-view">View Profile</button>
      </div>
    </div>`;
  }).join("");
  grid.querySelectorAll(".company-browse-card").forEach((card) =>
    card.querySelector(".cbc-view").addEventListener("click", () => {
...
function openCompanyPublic(c) {
  ...
  document.getElementById("pubSkills").innerHTML = (c.skills_required || []).map((s) => `<span class="skill-tag">${s}</span>`).join("") || '<span class="muted">Not specified</span>';
  const pct = c.employees_needed > 0 ? Math.min(100, Math.round((c.employees_have / c.employees_needed) * 100)) : 0;
  document.getElementById("pubTrackerFill").style.setProperty("--w", pct + "%");
  document.getElementById("pubTrackerLabel").textContent = `${c.employees_have || 0} / ${c.employees_needed || 0} positions filled`;
...
function openApplyModal(company) {
  ...
  document.getElementById("modalTitle").textContent = `Apply to ${company.org_name || "this company"}`;
  document.getElementById("modalBody").innerHTML = `
    <form id="applyForm">
  ...
  document.getElementById("modalOverlay").classList.add("show");
  document.getElementById("applyForm").addEventListener("submit", (e) => submitApplication(e, company));
}

async function submitApplication(e, company) {
  e.preventDefault();
  ...
  await supabase.from("notifications").insert({
    ...
    body: `Applied for a role at ${company.org_name}`,
    ...
    <tr>
      ...
      <td>
        <select class="status-select" data-status="${a.id}">
          ${["submitted", "viewed", "shortlisted", "rejected"].map((s) => `<option value="${s}" ${a.status === s ? "selected" : ""}>${s}</option>`).join("")}
        </select>
      </td>
      <td>${new Date(a.created_at).toLocaleDateString()}</td>
...
async function loadConversations() {
  ...
  el.innerHTML = allConversations.map((c) => {
    ...
    <div class="conv-item" data-id="${c.id}">
      <div class="conv-avatar">${name.slice(0, 2).toUpperCase()}</div>
