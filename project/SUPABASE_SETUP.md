...
3. Add environment variables under **Project → Settings → Environment Variables**:
   ...
   - `ANTHROPIC_API_KEY` (only needed for the real AI chat — see below)
...
`api/ai-chat.js` calls Anthropic's Claude API server-side, so the key never reaches
the browser. To turn it on:
